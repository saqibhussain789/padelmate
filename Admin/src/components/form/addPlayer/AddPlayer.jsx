  ...
  return (
    <div className="container mx-auto py-6 max-w-6xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Add a waiver notice that users need to accept upon visiting the
              club page.
            </p>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mb-4">
              You will receive an email notification for any of the selected
              events below based on your chosen preferences.
            </p>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {["Join", "Leave"].map((action) => (
                      <div key={action} className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          id={`${event.toLowerCase()}-${action.toLowerCase()}`}
                        />
                        <label
                          htmlFor={`${event.toLowerCase()}-${action.toLowerCase()}`}
                        >
                          {action}
                        </label>
                      </div>
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={item} className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor={item.toLowerCase().replace(/ /g, "-")}>
                    {item}
                  </label>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={sport} className="flex items-center space-x-2">
                  ...
                  <label htmlFor={`${sport.toLowerCase()}-sport`}>
                    {sport}
                  </label>
                </div>
              ))}
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mb-4">
              When enabled, these options are preselected upon creating an
              activity. However, you can still change the value per activity.
            </p>

            <div className="space-y-6">
              {[
                { type: "BOOKING", open: bookingOpenHours, close: bookingCloseHours, setOpen: setBookingOpenHours, setClose: setBookingCloseHours },
                { type: "TOURNAMENT", open: tournamentOpenHours, close: tournamentCloseHours, setOpen: setTournamentOpenHours, setClose: setTournamentCloseHours },
                { type: "TRAINING", open: trainingOpenHours, close: trainingCloseHours, setOpen: setTrainingOpenHours, setClose: setTrainingCloseHours },
                { type: "MATCHES", open: matchesOpenHours, close: matchesCloseHours, setOpen: setMatchesOpenHours, setClose: setMatchesCloseHours },
              ].map((event) => (
                <div key={event.type}>
                  <div className="flex items-center space-x-2 mb-2">
                    <input
                      type="checkbox"
                      id={`${event.type.toLowerCase()}-check-in`}
                    />
                    <label
                      htmlFor={`${event.type.toLowerCase()}-check-in`}
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4">
                        <button
                          className="h-10 w-10 border rounded-md"
                          onClick={() =>
                            event.setOpen((prev) => Math.max(0, prev - 1))
                          }
                        >
                          -
                        </button>
                        <span className="w-8 text-center">{event.open}</span>
                        <button
                          className="h-10 w-10 border rounded-md"
                          onClick={() => event.setOpen((prev) => prev + 1)}
                        >
                          +
                        </button>
                      </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-4">
                        <button
                          className="h-10 w-10 border rounded-md"
                          onClick={() =>
                            event.setClose((prev) => Math.max(0, prev - 1))
                          }
                        >
                          -
                        </button>
                        <span className="w-8 text-center">{event.close}</span>
                        <button
                          className="h-10 w-10 border rounded-md"
                          onClick={() => event.setClose((prev) => prev + 1)}
                        >
                          +
                        </button>
                      </div>
      ...
      <div className="mt-8 flex justify-end">
        <button className="bg-[#2a7b9b] text-white px-6 py-2 text-lg rounded-xl">
          Save
        </button>
      </div>
    </div>
